...
class Lexer:
    ...
    def _id(self):
        ...
        if self.current_char == '(' and result not in reserved_keyws and (self.previous_token.type != Tokens.FUNC if self.previous_token else True):
            ...
            token = reserved_keyws.get(
                result, Token(Tokens.ID, result, line, column))
        ...
        while self.current_char is not None and (self.current_char != char or escaped):
            ...
            if escaped:
                ...
                if self.current_char == '\\':
                    ...
                    result += [Token(Tokens.STRING,
                                     currentstring, line, column)]
                    currentstring = ''
                    self.advance()
                    result += [self._id()]
                    willadvance = False
        ...
        while self.current_char is not None:
            ...
            if self.current_char.isspace():
                ...
                self.current_token = Token(
                    Tokens.LESS_EQUAL, '<=', line, column)

            elif self.current_char == '>' and self.peek() == '=':
                self.advance()
                self.advance()
                self.current_token = Token(
                    Tokens.GREATER_EQUAL, '>=', line, column)
